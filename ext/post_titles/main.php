...
final class PostTitles extends Extension
{
    ...
    {
        $title = self::get_title($event->image);
        $heading = $title !== '' ? $title : "Post #{$event->image->id}";
        Ctx::$page->set_heading($heading);
        Ctx::$page->set_title($heading);
    }

    #[EventListener]
    public function onUploadCommonBuilding(UploadCommonBuildingEvent $event): void
    {
        $event->add_part($this->theme->get_title_upload_html(), 5);
    }

    #[EventListener(priority: 80)]
    public function onDataUpload(DataUploadEvent $event): void
    {
        $title = trim((string)($event->metadata['title'] ?? ''));
        if ($title === '') {
            return;
        }
        foreach ($event->posts as $post) {
            $this->set_title($post->id, $title);
        }
    }
