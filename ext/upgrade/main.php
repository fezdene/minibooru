...
final class Upgrade extends Extension
{
    ...
    public function onDatabaseUpgrade(DatabaseUpgradeEvent $event): void
    {
        ...
        if ($this->get_version() < 22) {
            Log::info("upgrade", "Adding sha256_hash column to images table");
            $database->execute("ALTER TABLE images ADD COLUMN sha256_hash VARCHAR(64) NULL");
            $this->set_version(22);
        }

        if ($this->get_version() < 23) {
            Log::info("upgrade", "Setting homepage as default front page");
            $database->execute(
                "INSERT OR REPLACE INTO config (name, value) VALUES ('front_page', 'home')"
            );
            $database->execute(
                "INSERT OR REPLACE INTO config (name, value) VALUES ('main_page', 'home')"
            );
            $this->set_version(23);
        }

        if ($this->get_version() < 24) {
            Log::info("upgrade", "Applying minibooru project defaults");

            // ── Theme, routing & homepage defaults ────────────────────────────
            foreach ([
                'theme'           => 'modernbooru',
                'nice_urls'       => 'Y',
                'tags_min'        => '1',
                'title'           => 'the archive.',
                'home_subtitle'   => 'A curated digital media archive.',
                'home_text'       => 'Equipped with rsync for efficient synchronization and fortified by secure hash algorithms to guarantee data integrity against link rot.',
                'home_links'      => 'https://www.linkedin.com/in/hafizuddin-hamsani/',
                'home_grad_dark'  => '#050a1a',
                'home_grad_mid'   => '#0d1b3e',
                'home_grad_glow'  => '#1d6ae5',
                'home_cta_text'   => 'Search',
                'home_show_recent'=> 'Y',
            ...
            // ── Extensions ────────────────────────────────────────────────────
            // Merge any existing extras with the full required set so this is
            // safe to run on both fresh installs and existing containers.
            $required = [
                'stats', 'custom_html_headers', 'perm_manager',
                'gallerydl_ingest', 'mass_remover', 'network_ops', 'sha256_check',
                'handle_audio', 'handle_cbz', 'handle_ico',
                'handle_image_extra', 'handle_pdf', 'handle_svg',
                'handle_video', 'handle_video_extra',
                'favorites', 'featured', 'filter', 'post_peek', 'zoom_to_fit',
                'word_filter', 'google_analytics', 'user_config_editor',
                'home',
            ];
            $merged = array_values(array_unique(
                array_merge(SysConfig::getExtraExtensions(), $required)
            ));
            $conf_dir = 'data/config';
            if (!is_dir($conf_dir)) {
                mkdir($conf_dir, 0750, true);
            }
            file_put_contents(
                $conf_dir . '/extensions.conf.php',
                "<?php\ndefine(\"EXTRA_EXTS\", " . json_encode($merged) . ");\n"
            );

            $this->set_version(24);
        }

        if ($this->get_version() < 25) {
            Log::info("upgrade", "Enabling handle_html extension");
            $required = ['handle_html'];
            $merged = array_values(array_unique(
                array_merge(SysConfig::getExtraExtensions(), $required)
            ));
            $conf_dir = 'data/config';
