...
final class UserPage extends Extension
{
    ...
    public function onInitExt(InitExtEvent $event): void
    {
        ...
        // Register ghost class here so ban works even when ipban extension is inactive.
        // ipban re-registers it with the same base if active — that's fine.
        if (!isset(UserClass::$known_classes['ghost'])) {
            new UserClass(
                'ghost',
                'base',
                [],
                description: 'Banned users can log in but have no posting permissions.',
            );
        }
    ...
    public function onPageRequest(PageRequestEvent $event): void
    {
        ...
        if ($event->page_matches("user_admin/list", method: "GET")) {
            $users = $database->get_all("
                SELECT u.id, u.name, u.class, COALESCE(u.email,'') AS email,
                       u.joindate, COUNT(i.id) AS post_count
                FROM users u
            ...
            try {
                $bans = $database->get_all("
                    SELECT b.id, b.ip, b.mode, b.reason, b.added, b.expires,
                           usr.name AS banner
                    FROM bans b
                    JOIN users usr ON b.banner_id = usr.id
                    WHERE (b.expires > CURRENT_TIMESTAMP OR b.expires IS NULL)
                    ORDER BY b.id DESC
                ");
            ...
            sort($classes);
            $page->set_title("User Manager");
            $page->set_heading("User Manager");
            $this->theme->display_user_manager($users, $bans, $classes);
        }
        if ($event->page_matches("user_admin/mgr_set_class", method: "POST")) {
            $id    = (int)($_POST['id']    ?? 0);
            $class = (string)($_POST['class'] ?? '');
            if ($id > 0 && $class !== '' && $user->class->name === 'admin') {
                $duser = User::by_id($id);
                $duser->set_class($class);
                $page->flash("Role updated for {$duser->name}.");
            }
            $page->set_redirect(make_link('user_admin/list'));
        }
        if ($event->page_matches("user_admin/mgr_delete", method: "POST", permission: UserAccountsPermission::DELETE_USER)) {
            $id = (int)($_POST['id'] ?? 0);
            if ($id > 0 && $id !== $user->id) {
                $duser = User::by_id($id);
                $name  = $duser->name;
                $anon  = Ctx::$config->get(UserAccountsConfig::ANON_ID);
                $database->execute("UPDATE images   SET owner_id=:a WHERE owner_id=:id", ['a' => $anon, 'id' => $id]);
                $database->execute("UPDATE comments SET owner_id=:a WHERE owner_id=:id", ['a' => $anon, 'id' => $id]);
                send_event(new UserDeletionEvent($id));
                $database->execute("DELETE FROM users WHERE id=:id", ['id' => $id]);
                Log::warning('user', "User #{$id} ({$name}) deleted by {$user->name}");
                $page->flash("User \"{$name}\" deleted.");
            }
            $page->set_redirect(make_link('user_admin/list'));
        }
        if ($event->page_matches("user_admin/mgr_ban", method: "POST", permission: IPBanPermission::BAN_IP)) {
            $ip      = trim((string)($_POST['ip']      ?? ''));
            $mode    = (string)($_POST['mode']    ?? 'block');
            $reason  = trim((string)($_POST['reason']  ?? ''));
            $expires = trim((string)($_POST['expires'] ?? ''));
            if ($ip !== '' && $reason !== '') {
                if (!in_array($mode, ['block','firewall','ghost','anon-ghost'], true)) {
                    $mode = 'block';
                }
                $expires_val = $expires !== '' ? date('Y-m-d H:i:s', (int)\Safe\strtotime($expires)) : null;
                send_event(new AddIPBanEvent(IPAddress::parse($ip), $mode, $reason, $expires_val));
                $page->flash("IP {$ip} banned ({$mode}).");
            } else {
                $page->flash('IP address and reason are required.');
            }
            $page->set_redirect(make_link('user_admin/list'));
        }
        if ($event->page_matches("user_admin/mgr_unban", method: "POST", permission: IPBanPermission::BAN_IP)) {
            $id = (int)($_POST['id'] ?? 0);
            if ($id > 0) {
                send_event(new RemoveIPBanEvent($id));
                $page->flash('Ban removed.');
            }
            $page->set_redirect(make_link('user_admin/list'));
        }
        if ($event->page_matches("user_admin/mgr_create", method: "POST", permission: UserAccountsPermission::CREATE_OTHER_USER)) {
            $name  = trim((string)($_POST['name']  ?? ''));
            $pass1 = (string)($_POST['pass1'] ?? '');
            $pass2 = (string)($_POST['pass2'] ?? '');
            $email = trim((string)($_POST['email'] ?? ''));
            $class = (string)($_POST['class'] ?? 'user');
            if ($name !== '' && $pass1 !== '') {
                try {
                    $uce = send_event(new UserCreationEvent($name, $pass1, $pass2, $email, false));
                    if (isset(UserClass::$known_classes[$class]) && $class !== 'user') {
                        $uce->get_user()->set_class($class);
                    }
                    $page->flash("User \"{$name}\" created.");
                } catch (\Exception $e) {
                    $page->flash('Error: ' . $e->getMessage());
                }
            } else {
                $page->flash('Username and password are required.');
            }
            $page->set_redirect(make_link('user_admin/list'));
        }
