...
=========================================================================
Scheduled via cron to run every 15 minutes on the Mirror Node.
...
def heal_file(master_ip: str, md5_hash: str, local_path: Path) -> bool:
    """
    Restore a corrupted or missing file by pulling it from the Master Node
    via RSYNC over SSH.

    Security:
    - subprocess.run() is called with a list (not a string), so shell=False
      (the default). No shell interpolation occurs regardless of master_ip.
    - master_ip comes from the admin-configured database value; it is not
      derived from any HTTP request or untrusted input at this point.
    - md5_hash is a 32-char hex string from the database — no special chars.

    Returns True if rsync exits 0 and the file now exists locally.
    """
    # Build remote path using the same warehouse formula as the local side.
    remote_parts = [REMOTE_DATA_DIR]
    for i in range(WH_SPLITS):
        remote_parts.append(md5_hash[i * 2 : i * 2 + 2])
    remote_parts.append(md5_hash)
    remote_path = f"{RSYNC_SSH_USER}@{master_ip}:{'/'.join(remote_parts)}"

    # Ensure the local parent directory exists (relevant for MISSING files).
    local_path.parent.mkdir(parents=True, exist_ok=True)

    cmd = [
        "rsync",
        "--archive",   # preserve permissions, timestamps, owner
        "--inplace",   # overwrite the corrupt bytes directly; avoids double-disk-use
        "--partial",   # keep partial transfer on interruption (resume on next run)
        "--checksum",  # force content comparison, not just size/mtime
        f"--timeout={RSYNC_TIMEOUT_SECS}",
        remote_path,
        str(local_path),
    ]

    logger.info("Self-heal: initiating rsync pull for '%s' from %s", md5_hash, master_ip)
    ...
    try:
        ...
        if result.returncode == 0:
            return True
        logger.error(
            "Self-heal rsync failed (exit %d): %s",
            result.returncode,
            result.stderr.strip(),
        )
        return False
    except subprocess.TimeoutExpired:
        logger.error("Self-heal rsync timed out (%ds) for '%s'", RSYNC_TIMEOUT_SECS, md5_hash)
        return False
    except FileNotFoundError:
        logger.error(
            "rsync binary not found. Install with: apt-get install rsync"
        )
        return False
...
#
# 6. Schedule in root's crontab (every 15 minutes):
#       crontab -e
#
#       Add this line:
#       */15 * * * * /usr/bin/python3 /opt/scripts/integrity_audit.py
#
#       To suppress all console output (cron email) and rely solely on the log:
#       */15 * * * * /usr/bin/python3 /opt/scripts/integrity_audit.py >/dev/null 2>&1
#
